...
public final class TypeCheckProcFactory {
	...
	protected static final Log LOG = LogFactory
			.getLog(TypeCheckProcFactory.class.getName());
	...
	public static HashMap<Node, Object> genExprNode(ASTNode expr,
		...
		opRules.put(new RuleRegExp("R2", HiveParser.Number + "%|"
				...
				+ "%|" + HiveParser.BigintLiteral + "%"), getNumExprProcessor());
		opRules.put(new RuleRegExp("R3", HiveParser.Identifier + "%|"
				...
				+ HiveParser.KW_STRUCT + "%"), getStrExprProcessor());
	...
	public static class NullExprProcessor implements NodeProcessor {
		...
		public Object process(Node nd, Stack<Node> stack,
			...
			ExprNodeDesc desc = TypeCheckProcFactory
	...
	public static class ColumnExprProcessor implements NodeProcessor {
		...
		public Object process(Node nd, Stack<Node> stack,
			...
			ASTNode expr = (ASTNode) nd;
			ASTNode parent = stack.size() > 1 ? (ASTNode) stack.get(stack
					.size() - 2) : null;
			RowResolver input = ctx.getInputRR();
			...
			if (isTableAlias) {
				...
				if (colInfo == null) {
					...
					if (input.getIsExprResolver()) {
						...
						ctx.setError(
							ErrorMsg.NON_KEY_EXPR_IN_GROUPBY.getMsg(exprNode),
							expr);
						return null;
					} else {
					...
					// It's a column.
					ExprNodeColumnDesc exprNodColDesc = new ExprNodeColumnDesc(
	...
	public static class DefaultExprProcessor implements NodeProcessor {
		...
		static {
			...
			specialFunctionTextHashMap.put(HiveParser.TOK_ISNOTNULL,
				"isnotnull");
		...
		static ExprNodeDesc getXpathOrFuncExprNodeDesc(ASTNode expr,
				boolean isFunction, ArrayList<ExprNodeDesc> children,
				TypeCheckCtx ctx) throws SemanticException,
			...
			if (funcText.equals(".")) {
				...
				ExprNodeConstantDesc fieldName = (ExprNodeConstantDesc) children
						.get(1);
						...
						try {
							if (columnType
								...
								value = new Float(constValue);
									...
									|| (columnType
											.equalsIgnoreCase(serdeConstants.STRING_TYPE_NAME) && !constType
											.equalsIgnoreCase(serdeConstants.BIGINT_TYPE_NAME))) {
		...
		public Object process2(String[] node, NodeProcessorCtx procCtx,
			...
			// Return nulls for conversion operators
			// if
			// (conversionFunctionTextHashMap.keySet().contains(expr.getType())
			// || specialFunctionTextHashMap.keySet().contains(
			// expr.getType())
			// || expr.getToken().getType() == HiveParser.CharSetName
			// || expr.getToken().getType() == HiveParser.CharSetLiteral) {
	...
	public static ExprNodeDesc genSelectNodeDesc(String[] node,
			TypeCheckCtx tcCtx) throws SemanticException {
		// TODO Auto-generated method stub

		// Object tab = new ExprNodeConstantDesc(TypeInfoFactory.stringTypeInfo,
		// node[0]);
		Object col = new ExprNodeConstantDesc(TypeInfoFactory.stringTypeInfo,
				node[1]);

		RowResolver input = tcCtx.getInputRR();
		String tableAlias = node[0];
		// NOTE: tableAlias must be a valid non-ambiguous table alias,
		// because we've checked that in TOK_TABLE_OR_COL's process
		// method.
		ColumnInfo colInfo = input.get(tableAlias, ((ExprNodeConstantDesc) col)
				.getValue().toString());
	...
	public static ExprNodeDesc genFilterNodeDesc(QueryGraph querygraph,
			TypeCheckCtx tcCtx) throws SemanticException {
		// TODO Auto-generated method stub

		HashMap<String, HashMap<String, String>> filterKeys = querygraph.getFilterKeys();

		List<Object> descList = new LinkedList<Object>();

		for (String table : filterKeys.keySet()) {

			for (String column : filterKeys.get(table).keySet()) {

				String key = filterKeys.get(table).get(column);

				Number v = null;
				try {
					if (key.endsWith("L")) {
						// Literal bigint.
						v = Long.valueOf(key.substring(0, key.length() - 1));
					} else if (key.endsWith("S")) {
						// Literal smallint.
						v = Short.valueOf(key.substring(0, key.length() - 1));
					} else if (key.endsWith("Y")) {
						// Literal tinyint.
						v = Byte.valueOf(key.substring(0, key.length() - 1));
					} else {
						v = Double.valueOf(key);
						v = Long.valueOf(key);
						v = Integer.valueOf(key);
					}
				} catch (NumberFormatException e) {
					// do nothing here, we will throw an exception in the
					// following block
				}

				// Object tab = new ExprNodeConstantDesc(
				// TypeInfoFactory.stringTypeInfo, table);
				Object col = new ExprNodeConstantDesc(
						TypeInfoFactory.stringTypeInfo, column);

				Object val = null;
				if (v == null) {
          val = new ExprNodeConstantDesc(
							TypeInfoFactory.stringTypeInfo, key);
        } else {
          val = new ExprNodeConstantDesc(v);
        }

				RowResolver input = tcCtx.getInputRR();
				String tableAlias = table;
				ColumnInfo colInfo = input.get(tableAlias,
					((ExprNodeConstantDesc) col).getValue().toString());

				Object colDesc = new ExprNodeColumnDesc(colInfo.getType(),
						colInfo.getInternalName(), colInfo.getTabAlias(),
						colInfo.getIsVirtualCol());

				ArrayList<ExprNodeDesc> children = new ArrayList<ExprNodeDesc>(
						2);
				children.add((ExprNodeDesc) colDesc);
				children.add((ExprNodeDesc) val);
				...
				if (inferTypes.contains(constType)
					...
					try {
						...
						// this exception suggests the precise type inference
						// did not succeed
						// we'll try again to convert it to double
						// however, if we already tried this, or the column is
						// NUMBER type and
				...
				Object desc = ExprNodeGenericFuncDesc.newInstance(
					fi.getGenericUDF(), children);
				descList.add(desc);
			}
		}

		Object finalDesc = descList.get(0);
		for (int i = 1; i < descList.size(); i++) {

			ArrayList<ExprNodeDesc> children = new ArrayList<ExprNodeDesc>(2);
			children.add((ExprNodeDesc) finalDesc);
			children.add((ExprNodeDesc) descList.get(i));

			String funcText = "and";
			FunctionInfo fi = FunctionRegistry.getFunctionInfo(funcText);
			finalDesc = ExprNodeGenericFuncDesc.newInstance(fi.getGenericUDF(),
				children);
		}

		return (ExprNodeDesc) finalDesc;
	}

	public static ExprNodeDesc genJoinNodeDesc(String[] node, TypeCheckCtx tcCtx)
			throws SemanticException {
		// TODO Auto-generated method stub
